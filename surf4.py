...
#3b create and assign x/y/X, Y/Z variables
x = np.arange(inp.exp_spr-6, inp.exp_spr+6, 1)
y = np.arange(inp.lo_tp, inp.hi_tp, 1)
X, Y = np.meshgrid(x, y)
Z = nr.net_map

#3c create colormap, (for each axis:) define subplot titles and create surface plot
...
#3d assign view angles to each surface plot (elevation, azimuth)
ax1.view_init(40, 55)
ax2.view_init(40, 35)
ax3.view_init(0, 90)
ax4.view_init(0, 180)
